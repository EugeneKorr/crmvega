...
export const UnifiedMessageBubble: React.FC<UnifiedMessageBubbleProps & {
    onReplyClick?: (id: string | number) => void;
    highlighted?: boolean;
}> = ({
    msg,
    isOwn,
    ...
    highlighted
        ...
        return (
            <div
                id={`msg-${msg.id}`}
                className={`message-bubble-container ${highlighted ? 'highlighted' : ''}`}
                style={{
                    display: 'flex',
            ...
            >
                {replyMessage && (
                    <div
                        onClick={() => onReplyClick && onReplyClick(replyMessage.id)}
                        style={{
                            fontSize: 12,
                            color: '#8c8c8c',
                            marginBottom: 4,
